...
opts <- options(knitr.kable.NA = "", fig.align='center', fig.dim=c(8, 8))
```
...
A variável comp_6 parece **não** contribuir significativamente para classificarmos o diagnóstico (utilizando $\alpha = 0.05$). Um novo modelo pode ser ajustado removendo essa variável, já que variáveis irrelevantes podem deteriorar o modelo.
...
table(pred1$diagnosis, pred1$predicted)

table(pred2$diagnosis, pred2$predicted)
...
As métricas não estão muito distantes entre si, quando olhamos para cada modelo.É importante lembrar que a acurácia é uma métrica que deve ser usada com cuidado, pois ela pode não representar bem a situação. 
...
```{r}
mean(pred1$diagnosis != pred1$predicted)
mean(pred2$diagnosis != pred2$predicted)
